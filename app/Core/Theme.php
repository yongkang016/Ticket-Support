...
class Theme
{
    ...
    {
        return sprintf('<link rel="shortcut icon" href="%s" />', asset(config('theme-settings.KT_THEME_ASSETS.favicon')));
    }
    ...
    function includeFonts()
    {
        ...
        foreach (config('theme-settings.KT_THEME_ASSETS.fonts') as $url) {
    ...
    function getGlobalAssets($type = 'js')
    {
        ...
        }, config('theme-settings.KT_THEME_ASSETS.global.'.$type));
    }
    ...
    function getVendors($type)
    {
        ...
        foreach (self::$vendorFiles as $vendor) {
            $vendors = config('theme-settings.KT_THEME_VENDORS.'.$vendor);
    ...
    {
        $type = config('theme-settings.KT_THEME_ICONS', 'duotone');
